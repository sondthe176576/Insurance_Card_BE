...
<html lang="en">
<head>
    <!-- Thiết lập tiêu đề và các thẻ meta cho trang -->
    <meta charset="UTF-8">
    ...
    <!-- Liên kết đến các thư viện CSS và JavaScript bên ngoài -->
...
<div class="main-content ml-64 p-6">
    <div class="container mx-auto mt-5">
        <div class="card p-8 shadow-lg rounded-lg bg-white animate__animated animate__fadeIn">
            ...
            <form class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6" method="get"
                  action="${pageContext.request.contextPath}/listRenewContract">
                <div>
                ...
                <div class="flex items-end">
                    <button type="submit"
                            class="btn bg-indigo-600 text-white py-2 px-4 rounded-md shadow-md hover:bg-indigo-700"><i
            ...
            <c:if test="${param.message != null}">
                <div id="notification"
                     class="notification ${param.status == 'true' ? 'notification-success' : 'notification-error'}">
                    <c:out value="${param.message}"/>
                    <button type="button" class="btn-close" onclick="document.getElementById('notification').remove()">
                        &times;
                    </button>
                </div>
            </c:if>

            <!-- Bảng hiển thị danh sách hợp đồng cần gia hạn -->
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white divide-y divide-gray-200 shadow overflow-hidden sm:rounded-lg">
                    <thead class="bg-gray-800 text-white">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-100 uppercase tracking-wider">No
                        </th>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-100 uppercase tracking-wider">Start
                            Date
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-100 uppercase tracking-wider">End
                            Date
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-100 uppercase tracking-wider">Days
                            Overdue
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-100 uppercase tracking-wider">
                            Insurance Type
                    ...
                    <!-- Lặp qua danh sách hợp đồng cần gia hạn và hiển thị thông tin -->
                    <c:forEach var="entry" items="${contractDaysOverdueMap}" varStatus="status">
                        ...
                        <tr class="hover:bg-gray-100 transition duration-150 ease-in-out">
                            <td class="px-6 py-4 whitespace-nowrap"><c:out
                                    value="${(currentPage - 1) * 10 + status.count}"/></td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap"><fmt:formatDate value="${contract.startDate}"
                                                                                    pattern="yyyy-MM-dd"/></td>
                            <td class="px-6 py-4 whitespace-nowrap"><fmt:formatDate value="${contract.endDate}"
                                                                                    pattern="yyyy-MM-dd"/></td>
                            <!-- Hiển thị số ngày quá hạn bằng màu đỏ -->
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right"><fmt:formatNumber
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <a href="${pageContext.request.contextPath}/renewContract?contractID=${contract.contractID}"
                                   class="text-indigo-600 hover:text-indigo-900"><i class="fas fa-sync-alt"></i>
                                    Renew</a>
                            </td>
                        </tr>
                    </c:forEach>
